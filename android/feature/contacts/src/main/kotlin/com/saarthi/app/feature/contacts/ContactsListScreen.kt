...
fun ContactsListScreen(viewModel: ContactsViewModel = hiltViewModel()) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(20.dp)) {
        Text(text = "Call Someone", style = MaterialTheme.typography.headlineMedium)
        Spacer(modifier = Modifier.height(16.dp))

        if (contacts.isEmpty()) {
            Text(
                text = "No people yet. Add family so you can say \"call beta\".",
        ...
        Spacer(modifier = Modifier.height(16.dp))
        SaarthiButton(text = "➕ Add a person", onClick = { showAdd = true })
    }
...
private fun ContactRow(contact: Contact, onCall: () -> Unit, onWhatsApp: () -> Unit, onDelete: () -> Unit) {
    SaarthiCard {
        Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
            ...
            TextButton(onClick = onCall) { Text("📞") }
            TextButton(onClick = onWhatsApp) { Text("🟢") }
