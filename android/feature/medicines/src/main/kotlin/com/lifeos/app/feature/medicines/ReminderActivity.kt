            ...
            LifeOSTheme {
                Surface(modifier = Modifier.fillMaxSize()) {
                    ReminderConfirmScreen(
                        ...
                        onSnooze = {
                            // A user-requested snooze restarts the escalation ladder — it's a
                            // deliberate "ask me again," not an unacknowledged automatic nag.
                            scheduler.nagAgain(medicineId, time, attempt = 1, delayMs = 5 * 60 * 1000L)
                            notifier.cancel(medicineId, time)
                            finishAndRemoveTask()
                        },
                    )
...
private fun ReminderConfirmScreen(
    ...
    Column(
        ...
        LifeOSButton(text = "✅ I took it", onClick = onTaken)
        Spacer(modifier = Modifier.height(12.dp))
        LifeOSButton(text = "⏰ Remind me in 5 minutes", onClick = onSnooze)
    }
}
